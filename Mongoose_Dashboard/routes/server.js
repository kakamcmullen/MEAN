...
mongoose.connect('mongodb://localhost/wolf_db');// tester is the name of our database in mongodb
...
// Routes go here!
app.get('/', function (req, res) {
    Wolf.find({}, function (err, results) {
        if (err) { console.log(err); }
        res.render('index', { wolf: results });
    });
});
...
app.post('/', function (req, res) {
    // Create a new dog!
    Wolf.create(req.body, function (err, result) {
        if (err) { console.log(err); }
        res.redirect('/')
    });
});
...
app.get('/:id', function (req, res) {
    Wolf.find({ _id: req.params.id }, function (err, response) {
        if (err) { console.log(err); }
        res.render('show', { wolf: response[0] });
    });
});
...
app.get('/:id/edit/', function (req, res) {
    Wolf.find({ _id: req.params.id }, function (err, response) {
        if (err) { console.log(err); }
        res.render('edit', { wolf: response[0] });
    })
});
